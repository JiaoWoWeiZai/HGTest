...
namespace CrHgWcfService.Model
{
    public class FeeInfo : BaseInfo
    {
        ...
        public static bool GetFeeInfoFromHis(string registerId, string totalAmt,ref FeeInfo[] feeInfos, ref string errorMsg)
        {
            if (feeInfos == null) throw new ArgumentNullException(nameof(feeInfos));
            ...
            try
            {
                ...
                if (param2.Value.ToString() != "0")
                {
                    errorMsg = param3.Value.ToString();
                    return false;
                }
                feeInfos = (from DataRow row in db.Tables[0].Rows
                            select new FeeInfo
                            {
