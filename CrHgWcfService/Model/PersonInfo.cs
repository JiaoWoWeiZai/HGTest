...
namespace CrHgWcfService.Model
{
    public class PersonInfo : BaseInfo
    {
        ...
        public static PersonInfo GetPersonInfo(int pInter)
        {
            HgEngine.SetResultSet(pInter, "PersonInfo");
            var personInfo = new PersonInfo();

            foreach (var info in personInfo.GetType().GetProperties())
            {
                var value = string.Empty;
                HgEngine.GetByName(pInter, info.Name, ref value);
                info.SetValue(personInfo, value, null);
            }
